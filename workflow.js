// Breakage Incident workflow v45: Admin -> SPV -> Master
(function(){
  function stClass(st){st=String(st||'').toUpperCase();if(['FINAL','APPROVED_SPV'].includes(st))return 's-good';if(st==='RETURNED_ADMIN')return 's-bad';if(st==='MASTER_REVIEW')return 's-blue';return 's-watch'}
  ...
  renderIncidentPage=function(){
    ...
    <div class="hint" style="margin-bottom:10px"><b>Workflow:</b> Admin RDC Input Draft → SPV Submit/Approve / Return → Master Review / Adjustment → Final. Adjustment Master selalu tercatat di audit trail.</div>
    <div class="tablewrap"><table class="tbl"><thead><tr><th>Incident</th><th>Tanggal</th><th>RDC</th><th>Jenis</th><th>Item</th><th>Qty</th><th>No BA</th><th>Reported By</th><th>Status</th><th>Workflow</th></tr></thead><tbody>${rows.length?rows.map(r=>{
      ...
      if(ACCESS?.is_master&&st==='APPROVED_SPV'){
        act.push(`<button class="secondary" style="padding:6px 8px;font-size:10px" onclick="masterIncidentAction(${r.incident_id},'START_REVIEW')">Review</button>`);
  ...
  async function adjustIncident(id){
    ...
    let item=prompt('Kode Item:',String(r.item_code??''));if(item===null)return;
    let ba=prompt('No BA:',String(r.no_ba??''));if(ba===null)return;
    let cause=prompt('Cause/Penyebab:',String(r.cause??''));if(cause===null)return;
    ...
    let changes={qty_box:qty,item_code:item,no_ba:ba,cause:cause,responsibility:responsibility,pic:pic};
