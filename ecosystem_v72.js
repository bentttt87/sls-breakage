...
(function(){
  ...
  function baGroups(){
    ...
    (Array.isArray(INCIDENTS)?INCIDENTS:[]).filter(r=>String(r.incident_type||'').toLowerCase()==='delivery').forEach(r=>{
      const key=baGroupKey(r);
      if(!map.has(key))map.set(key,{key,rdc:r.rdc,occurrence_date:r.occurrence_date,no_sj:r.no_sj,factory:r.factory,customer:r.customer,receiver_name:r.ba_receiver_name,transporter:r.transporter,driver_name:r.driver_name,vehicle_no:r.vehicle_no,witness_name:r.ba_witness_name,items:[]});
      map.get(key).items.push(r);
    });
    return [...map.values()].map(g=>{
      g.qty=g.items.reduce((a,b)=>a+Number(b.qty_box||0),0);
      g.refs=[...new Set(g.items.map(x=>x.no_ba).filter(Boolean))];
      const st=g.items.map(x=>String(x.status||'').toUpperCase());
      g.status=st.every(x=>['FINAL','CLOSED'].includes(x))?'FINAL':st.every(x=>['APPROVED_SPV','MASTER_REVIEW','FINAL','CLOSED'].includes(x))?'VERIFIED':'OPEN';
      return g;
  ...
  function renderBaRecap(){
    if(!$('baTable'))return;
    let groups=baGroups();
    const q=String($('baSearch')?.value||'').trim().toLowerCase(),sf=$('baStatusFilter')?.value||'ALL';
    ...
    const all=baGroups();
    $('baCount').textContent=fmt(all.length);$('baItems').textContent=fmt(all.reduce((a,g)=>a+g.items.length,0));$('baQty').textContent=fmt(all.reduce((a,g)=>a+g.qty,0));
  ...
  window.printBaGroup=function(key){
    const g=baGroups().find(x=>x.key===key);
    if(!g){alert('Data BA tidak ditemukan.');return;}
    const storageKey='sls_ba_print_'+Date.now()+'_'+Math.random().toString(36).slice(2,8);
    ...
    const w=window.open('/ba_print.html?k='+encodeURIComponent(storageKey),'_blank');
    if(!w)alert('Popup diblokir browser. Izinkan popup untuk membuka BA + foto.');
  };
